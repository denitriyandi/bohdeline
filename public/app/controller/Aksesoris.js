Ext.define('CrudExt.controller.Aksesoris',{
	extend: 'Ext.app.Controller',
	views: ['aksesoris.Grid', 'aksesoris.Form', 'aksesoris.Window'],
	models: ['Aksesoris'],
	stores: ['Aksesoris'],

	refs: [{
            ref: 'list',
            selector: 'aksesorisGrid'
        }],

	init: function(){
		this.control({
            'aksesorisGrid button[action=edit]': {
                click: this.edit
            },
            'aksesorisGrid': {
                itemdblclick: this.edit
            },
            'aksesorisGrid button[action=add]': {
                click: this.add
            },
            'aksesorisGrid button[action=delete]': {
                click: this.destroy
            },
            'aksesorisForm button[action=save]': {
                click: this.save
            },
           
            'aksesorisGrid #aksesorisGridTrigger' : {
	...
	add: function(){
            var view = Ext.widget('aksesorisEdit');
	...
	edit: function(btn){
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('aksesorisEdit');
                var form = view.down('aksesorisForm').getForm();
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.Aksesoris');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
