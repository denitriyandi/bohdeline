Ext.define('CrudExt.controller.PurchaseOrders',{
	extend: 'Ext.app.Controller',
	views: ['purchasingOrder.Grid', 'purchasingOrder.Form', 'purchasingOrder.Window'],
	models: ['PurchaseOrders'],
	stores: ['PurchaseOrders'],

	refs: [{
            ref: 'list',
            selector: 'purchasingOrderGrid'
        }],

	init: function(){
		this.control({
            'purchasingOrderGrid button[action=edit]': {
                click: this.edit
            },
            'purchasingOrderGrid': {
                itemdblclick: this.edit
            },
            'purchasingOrderGrid button[action=add]': {
                click: this.add
            },
            'purchasingOrderGrid button[action=delete]': {
                click: this.destroy
            },
            'purchasingOrderForm button[action=save]': {
                click: this.save
            },
           
            'purchasingOrderGrid #purchasingOrderGridTrigger' : {
	...
	add: function(){
            var view = Ext.widget('purchasingOrderEdit');
	...
	edit: function(btn){
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('purchasingOrderEdit');
                var form = view.down('purchasingOrderForm').getForm();
	...
	save: function(btn){
            var form = btn.up('purchasingOrderForm');
            var win = form.up('window');
            var basicForm = form.getForm();
            var grid = this.getList();
            var store = Ext.create('CrudExt.store.PurchaseOrders');
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.PurchaseOrders');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
