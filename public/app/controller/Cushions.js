Ext.define('CrudExt.controller.Cushions',{
	extend: 'Ext.app.Controller',
	views: ['cushion.Grid', 'cushion.Form', 'cushion.Window'],
	models: ['Cushions'],
	stores: ['Cushions'],

	refs: [{
            ref: 'list',
            selector: 'cushionGrid'
        }],

	init: function(){
		this.control({
            'cushionGrid button[action=edit]': {
                click: this.edit
            },
            'cushionGrid': {
                itemdblclick: this.edit
            },
            'cushionGrid button[action=add]': {
                click: this.add
            },
            'cushionGrid button[action=delete]': {
                click: this.destroy
            },
            'cushionForm button[action=save]': {
                click: this.save
            },
            'cushionGrid #cushionGridTrigger':{
	...
	add: function(){
            var view = Ext.widget('cushionEdit');
            view.setTitle('Tambah Cushion');
	},
	edit: function(btn){
            var grid = this.getList();
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('cushionEdit');
                var form = view.down('cushionForm').getForm();

                form.loadRecord(record);
                view.setTitle('Edit Cushion');
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.Cushions');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
