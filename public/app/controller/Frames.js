Ext.define('CrudExt.controller.Frames',{
	extend: 'Ext.app.Controller',
	views: ['frame.Grid', 'frame.Form', 'frame.Window'],
	models: ['Frames'],
	stores: ['Frames'],
	...
	init: function(){
		this.control({
            'frameGrid button[action=edit]': {
                click: this.edit
            },
            'frameGrid': {
                itemdblclick: this.edit
            },
            'frameGrid button[action=add]': {
                click: this.add
            },
            'frameGrid button[action=delete]': {
                click: this.destroy
            },
            'frameForm button[action=save]': {
                click: this.save
            },
            'frameGrid #frameGridTrigger':{
	...
	add: function(){
            var view = Ext.widget('frameEdit');
            view.setTitle('Tambah frame');
	},
	edit: function(btn){
            var grid = this.getList();
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('frameEdit');
                var form = view.down('frameForm').getForm();

                form.loadRecord(record);
                view.setTitle('Edit frame');
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.Frames');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
