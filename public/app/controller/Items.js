Ext.define('CrudExt.controller.Items',{
	extend: 'Ext.app.Controller',
	views: ['item.Grid', 'item.Form', 'item.Window'],
	models: ['Items'],
	stores: ['Items'],
	...
	init: function(){
		this.control({
            'itemGrid button[action=edit]': {
                click: this.edit
            },
            'itemGrid': {
                itemdblclick: this.edit
            },
            'itemGrid button[action=add]': {
                click: this.add
            },
            'itemGrid button[action=delete]': {
                click: this.destroy
            },
            'itemForm button[action=save]': {
                click: this.save
            },
            'itemGrid #itemGridTrigger':{
	...
	edit: function(btn){
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('itemEdit');
                var form = view.down('itemForm').getForm();
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.Items');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
