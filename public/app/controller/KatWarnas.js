Ext.define('CrudExt.controller.KatWarnas',{
	extend: 'Ext.app.Controller',
	views: ['katWarna.Grid', 'katWarna.Form', 'katWarna.Window'],
	models: ['KatWarnas'],
	stores: ['KatWarnas'],

	refs: [{
            ref: 'list',
            selector: 'katWarnaGrid'
        }],

	init: function(){
		this.control({
            'katWarnaGrid button[action=edit]': {
                click: this.edit
            },
            'katWarnaGrid': {
                itemdblclick: this.edit
            },
            'katWarnaGrid button[action=add]': {
                click: this.add
            },
            'katWarnaGrid button[action=delete]': {
                click: this.destroy
            },
            'katWarnaForm button[action=save]': {
                click: this.save
            },
            'katWarnaGrid #katwarnaGridTrigger':{
	...
	add: function(){
            var view = Ext.widget('katWarnaEdit');
            view.setTitle('Tambah Kategori Warna');
	},
	edit: function(btn){
            var grid = this.getList();
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('katWarnaEdit');
                var form = view.down('katWarnaForm').getForm();

                form.loadRecord(record);
                view.setTitle('Edit Kategori Warna');
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.KatWarnas');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
