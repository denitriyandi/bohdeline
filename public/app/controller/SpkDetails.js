Ext.define('CrudExt.controller.SpkDetails',{
	extend: 'Ext.app.Controller',
	views: ['spkDetail.Grid', 'spkDetail.Form', 'spkDetail.Window'],
	models: ['SpkDetails'],
	stores: ['SpkDetails'],

	refs: [{
            ref: 'list',
            selector: 'spkDetailGrid'
        }],

	init: function(){
		this.control({
            'spkDetailGrid button[action=edit]': {
                click: this.edit
            },
            'spkDetailGrid': {
                itemdblclick: this.edit
            },
            'spkDetailGrid button[action=add]': {
                click: this.add
            },
            'spkDetailGrid button[action=delete]': {
                click: this.destroy
            },
            'spkDetailForm button[action=save]': {
                click: this.save
            },
           
            'spkDetailGrid #spkDetailGridTrigger' : {
	...
	add: function(){
            var view = Ext.widget('spkDetailEdit');
	...
	edit: function(btn){
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('spkDetailEdit');
                var form = view.down('spkDetailForm').getForm();
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.SpkDetails');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
