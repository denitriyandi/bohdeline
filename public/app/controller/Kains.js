Ext.define('CrudExt.controller.Kains',{
	extend: 'Ext.app.Controller',
	views: ['kain.Grid', 'kain.Form', 'kain.Window'],
	models: ['Kains'],
	stores: ['Kains'],
	...
	init: function(){
		this.control({
            'kainGrid button[action=edit]': {
                click: this.edit
            },
            'kainGrid': {
                itemdblclick: this.edit
            },
            'kainGrid button[action=add]': {
                click: this.add
            },
            'kainGrid button[action=delete]': {
                click: this.destroy
            },
            'kainForm button[action=save]': {
                click: this.save
            },
           
            'kainGrid #kainGridTrigger' : {
	...
	edit: function(btn){
            ...
            if(records.length === 1){
                ...
                var view = Ext.widget('kainEdit');
                var form = view.down('kainForm').getForm();
	...
	save: function(btn){
            ...
            if(basicForm.isValid()){
                if(!record){
                    record = Ext.create('CrudExt.model.Kains');
                    record.set(values);
                    store.add(record);
                }else{
                    record.set(values);
